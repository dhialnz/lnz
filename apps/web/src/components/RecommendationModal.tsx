    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between p-5 border-b border-border">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className={cn("text-xs font-mono font-semibold", riskColor(rec.risk_level))}>{rec.risk_level} Risk</span>
              <span className="text-xs font-mono text-muted">{rec.category}</span>
              <span className="text-xs font-mono text-muted">Confidence: {(rec.confidence * 100).toFixed(0)}%</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-5">
          <section>
            <h3 className="text-xs font-mono text-muted uppercase tracking-wider mb-2">Explanation</h3>
            <LLMText text={rec.explanation} lineClassName="text-sm text-gray-300 leading-relaxed" className="space-y-1" />
          </section>

          {rec.triggers.length > 0 && (
            <section>
              <h3 className="text-xs font-mono text-muted uppercase tracking-wider mb-2">Rule Triggers</h3>
          ...
          {Object.keys(rec.supporting_metrics).length > 0 && (
            <section>
              <h3 className="text-xs font-mono text-muted uppercase tracking-wider mb-2">Supporting Metrics</h3>
          ...
          {rec.actions.length > 0 && (
            <section>
              <h3 className="text-xs font-mono text-muted uppercase tracking-wider mb-2">Suggested Actions</h3>
              <ol className="space-y-2">
                {rec.actions.map((action, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-gray-300">
                    <span className="font-mono text-accent text-xs mt-0.5 flex-shrink-0">{String(i + 1).padStart(2, "0")}.</span>
                    <LLMText text={action} className="space-y-0" lineClassName="text-sm text-gray-300 leading-6" />
                  </li>
                ))}
              </ol>
            </section>
          ...
          <p className="text-xs font-mono text-muted border-t border-border pt-3">Generated: {fmtDate(rec.created_at)}</p>
        </div>
      </div>
    </div>
  );

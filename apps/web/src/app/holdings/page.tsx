...
import { addHolding, deleteHolding, getHoldings, updateHolding } from "@/lib/api";
import type { HoldingIn, HoldingLive, HoldingsSnapshot } from "@/lib/types";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        />
        <MetricTile
          label={`Day Change (${currencyLabel})`}
          value={
            s.total_day_change != null
              ? fmtDisplayCurrency(fromUsd(s.total_day_change), 0, { forceSign: true })
              : "-"
          }
          valueClass={signedClass(s.total_day_change)}
          sub={
            s.total_day_change_pct != null
              ? `${signedPfx(s.total_day_change_pct)}${fmtPct(s.total_day_change_pct)}`
              : undefined
          }
        />
        <MetricTile
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 min-w-0">
        ...
        <div className="lg:col-span-3 min-w-0 bg-panel border border-border rounded-lg p-4">
          ...
          {s.holdings.length === 0 && !showForm ? (
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-[62vh] rounded border border-border/40">
                  ...
                  {sorted.map((h) => (
                    <tr key={h.id} className="border-b border-border/40 hover:bg-surface/50 transition-colors">
                      ...
                      <td className="py-2.5 text-right">
                        <button
                          onClick={() => startEdit(h)}
                          className="text-muted hover:text-accent mr-2 transition-colors"
                          title="Edit"
                        >
                          Edit
                        </button>
